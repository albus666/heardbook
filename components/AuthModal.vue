        ...
        <div class="modal__resizer">
          ...
          <div class="modal__container" ref="container" :style="{ height: containerHeight }">
            ...
            <div class="modal__step" ref="modalStep"
                 :class="{ 'modal__step--fading-out': isFadingOut, 'modal__step--transitioning': isTransitioning }">
              <transition name="content-slide" mode="out-in">
                <!-- ==================================================================================================== -->
                <!-- SIGNUP FORM (Adapted from SignUpModal.vue) -->
                <!-- ==================================================================================================== -->
                <div v-if="tempMode === 'signup'" key="signup"
                     :class="['auth-popup-form', 'auth-popup-form--signup', { 'content-fade-out': isFadingOut && tempMode === 'signup' }]">
                ...
                <!-- ==================================================================================================== -->
                <div v-else-if="tempMode === 'login'" key="login"
                     :class="['login-modal-form', { 'content-fade-out': isFadingOut && tempMode === 'login' }]">
                  <div class="login-modal-form__container">
                    ...
                    <div class="login-modal-form__body login-form-body">
                      <form @submit.prevent="handleLogin" class="login-form">
                        ...
                        <!-- Submit Button -->
                        <button type="submit"
                                class="login-form__button login-form__button--full login-form__submit-btn">
                    ...
                    <div class="login-modal-form__body login-form-body">
                      <form class="login-form">
                        <div class="login-form__social-logins">
                          ...
                          <div class="login-form__social-logins-row">
                            <a class="login-form__button login-form__button--midnight login-form__sso-button"
                               href="/auth/b2b">
                              Log in with SSO
                            </a>
                          </div>
...
export default {
  ...
  methods: {
        ...
        requestAnimationFrame(() => {
          requestAnimationFrame(() => {
            ...
            if (Math.abs(newHeight - oldHeight) > 1) {
              ...
              if (this.$refs.modalStep) {
                const oldContent = this.$refs.modalStep.querySelector('.auth-popup-form, .login-modal-form')
                if (oldContent) {
                  // 清除内联样式，让 CSS transition 生效
                  oldContent.style.opacity = ''
                  oldContent.style.transform = ''
                }
              }
              
              // 2. 强制 reflow，确保样式清除生效
              ...
              requestAnimationFrame(() => {
                ...
                // 等待新内容渲染，然后开始高度动画
                this.$nextTick(() => {
                  // 确保新内容也是透明状态（清除可能的内联样式）
                  if (this.$refs.modalStep) {
                    const newContent = this.$refs.modalStep.querySelector('.auth-popup-form, .login-modal-form')
                    if (newContent) {
    ...
    ensureContentVisible() {
      ...
      // 注意：只在非过渡状态下设置内联样式，避免干扰 CSS transition
      if (this.$refs.modalStep && !this.isTransitioning) {
        const content = this.$refs.modalStep.querySelector('.auth-popup-form, .login-modal-form')
        if (content) {
